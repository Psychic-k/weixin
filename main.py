...
def rhetoric():
    if (Whether_rhetoric != False):
        try:
            ...
            return ("情诗API调取错误，请检查API是否正确申请或是否填写正确")
...
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips,
                 note_en, note_ch, health_tip, lucky_, rhetoric):
...
if __name__ == "__main__":
    ...
    # 是否启用情诗语录API
    Whether_rhetoric = config["Whether_rhetoric"]
    # 获取星座
    astro = config["astro"]
    # 获取词霸每日金句
